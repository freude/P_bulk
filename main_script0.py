...
amp = read_env1(x, bands, os.path.join(pth, '/dis_scr/'), kk[0, :], 0, [0, 0, 0])

amp[np.abs(amp) < 0.01 * np.max(np.abs(amp))] = 0.0

amp = np.vstack((amp, amp, amp, amp, amp, amp)).T
        ...
        for jj1 in xrange(Nbands):
            for jj2 in xrange(Nbands):
                if (j1 == j2) and (jj1!=jj2):
                    ...
                    M[jj1 + Nbands * (j1 - 1), jj2 + Nbands * (j2 - 1)] = MEs[j1, j2] * amp[j1, jj1] * amp[j2, jj2]
