...
% system('FreeFem++ /data/users/mklymenko/abinitio_software/abi_files/tpaw/make_mesh.edp');
...
% [~,V1]=pot_for_ff(kk(1,:), kk(1,:),'1');
% [~,V2]=pot_for_ff(kk(2,:), kk(2,:),'2');
% [x,V3]=pot_for_ff(kk(3,:), kk(3,:),'3');

% system('FreeFem++ /data/users/mklymenko/abinitio_software/abi_files/tpaw/si_ham.edp 0 0 0.19 1 1');
% system('FreeFem++ /data/users/mklymenko/abinitio_software/abi_files/tpaw/si_ham.edp 0 0 1 0.19 1');
% system('FreeFem++ /data/users/mklymenko/abinitio_software/abi_files/tpaw/si_ham.edp 0 0 1 1 0.19');
...
for jj=1:num_bs
    ...
    for j2=1:Nbands
        for j3=1:6
            ...
            F=F+alt*EigVec1(j2+Nbands*(j3-1),jj).*squeeze(M1(j2,jjj,:,:,:)).*squeeze(wf1(j3,:,:,:,:)).*exp(1i*(kk(j3,1).*X+kk(j3,2).*Y+kk(j3,3).*Z));
        end;
        
    end;
    ME=trapz(x,trapz(x,trapz(x,abs(F).^2,3),2),1);
    
    if (max(real(F(:)))>max(imag(F(:))))
...
bs=load([pwd,'/dis_scr/bas_fun.mat']);
bas_fun = [bs.bas_fun; bas_fun(2:end)];


save([pwd,'/dis_scr/bas_fun.mat'], 'bas_fun', '-v7.3');

load([pwd,'/dis_scr/M.mat'],'a1');
a1=[a1; spectrum*40; spectrum*40; spectrum*40; spectrum*40; spectrum*40; spectrum*40];
[a1,~] = sort(a1);
save([pwd,'/dis_scr/M.mat'],'-append','a1');
